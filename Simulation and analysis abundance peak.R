...
#In this script, we simulate census data for amphibians and reptiles g1 abundance
#peak under various scenarios involving the number of road transects, as well as
#the variability in both the daily number of roadkills (lambda_itd) and the daily
#carcass persistence probability (ppd), as described in Section 2.4 of the main 
#text of the paper.
...
#persistence (pp), and carcass observation per per walking, cycling and driving
#survey method (pom) probabilities survey method (pom) probabilities. We use in 
#the analysis standard errors for thea pp prior set at either 0.05 or 0.1.
...
#3. Generate Carcass Persistence Curve for pp Prior Based on Santos et al. (2011)
...
#5. Census data simulation on SD lambda_td 0.5 and SD ppd 0.05: This section 
...
#applies the model to estimate the total number of roadkills, pp and pom 
...
#6. Census data simulation on SD lambda_td 0.5 and SD ppd 0.15
#7. Census data simulation on SD lambda_td 0.5 and SD ppd 0
#8. Census data simulation on SD lambda_td 1.5 and SD ppd 0.05
#9. Census data simulation on SD lambda_td 1.5 and SD ppd 0.15
#10. Census data simulation on SD lambda_td 1.5 and SD ppd 0
#11. Census data simulation on SD lambda_td 0 and SD ppd 0.05
#12. Census data simulation on SD lambda_td 0 and SD ppd 0.15
#13. Census data simulation on SD lambda_td 0 and SD ppd 0
...
setwd("C:/Users/USUARIO/Documents/ESTUDIOS/Master/Conservación Pablo Olavide/TFM/Analisis datos/CSV para Github") # set path to location where all files necessary to run the script are
...
# Standard Error for Carcass Persistence Probability (pp) prior distribution (modify for different scenarios)
...
select_group <- function(group_name) {
  if (group_name == "amphibians") {
    anual_tendency <- c("anual_tendency_amphibians.csv")
    ppd_group <- 0.392598565 # Daily carcass persistence probability
    pom_group <- c(0.5, 0.3, 0.02) # Carcass observation probabilities for each survey method (walking, cycling, driving)
  } else if (group_name == "reptiles_g1") {
    anual_tendency <- c("anual_tendency_reptiles_g1.csv")
...
group <- "reptiles_g1"  # write the group of interest
selected_group <- select_group(group)

# Directory to save simulation results (adjust to your specific path)
save_path <- "C:/Users/USUARIO/Documents/ESTUDIOS/Master/Conservación Pablo Olavide/TFM/Resultados/Simulaciones publicacion/Prueba/"
...
# --------------------------------------------------------------------------------------------
#3. Generate Carcass Persistence Curve for pp Prior Based on Santos et al. (2011) Daily pp (ppd)
# --------------------------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
#5. Census data simulation on SD lambda_td 0.5 and SD ppd 0.05
# -----------------------------------------------------------------------------
...
for (sim in 1:nsimulations) {
  ...
  saveRDS(random_values, file = paste0(save_path, group, "_SE_pp_", se_pp, "_nroad_transects_", nroad_transects, 
                                       "_SD_N_", sd_random_values, "_sd_ppd_", sd_ppd, 
                                       "_sd_random_values_sim_", sim, ".RData"))
  
  # Generate and save simulated lambdas_td (lambdasim)
  for (i in 1:D) {
    lambdasim <- monthly_trend * random_values[i]  # Adjusting growth rate with random values
    assign(paste0("lambdasim", i), lambdasim)  # Assign lambda value dynamically
    
    # Save lambdasim for each time step i
    saveRDS(lambdasim, file = paste0(save_path, group, "_SE_pp_", se_pp, "_nroad_transects_", nroad_transects, 
                                     "_SD_N_", sd_random_values, "_SD_ppd_", sd_ppd, 
  ...
  for (i in 1:D) {
    lambdasim_name <- paste0("lambdasim", i)
    ...
    # Save the matrix
    assign(paste0("N_itd", i), N_itd)
    saveRDS(N_itd, file = paste0(save_path, group, "_SE_pp_", se_pp, "_nroad_transects_", nroad_transects, 
                                 "_SD_N_", sd_random_values, "_SD_ppd_", sd_ppd, 
                                 "_N_itd", i, "_sim_", sim, ".RData"))
  }
  
  # Generate random daily carcass persistence probabilities along sd_ppd
  ppd <- rnorm(D, mean = selected_group$ppd_group, sd = sd_ppd)
  
  # Ensure the product of all ppd values is greater than 0.05
  while (prod(ppd) <= 0.05) {
    ppd <- rnorm(D, mean = selected_group$ppd_group, sd = sd_ppd)
  }
  
  # Save the generated ppd values
  saveRDS(ppd, file = paste0(save_path, group, "_SE_pp_", se_pp, "_nroad_transects_", nroad_transects, 
                             "_SD_N_", sd_random_values, "_SD_ppd_", sd_ppd, 
                             "_ppd_sim_", sim, ".RData"))
  ...
  for (j in 1:D) {
    ...
    for (i in 1:nroad_transects) {
      for (t in 1:nmonth) {
        N3_itd[i, t] <- rbinom(1, N_itd_current[i, t], ppd_t)  # equation 3
      }
    }
    
    # Save the N3_itd matrix for each day in D-day period
    assign(paste0("N3_itd", j), N3_itd)
    saveRDS(N3_itd, file = paste0(save_path, group, "_SE_pp_", se_pp, "_nroad_transects_", nroad_transects, 
                                  "_SD_N_", sd_random_values, "_SD_ppd_", sd_ppd, 
  ...
  saveRDS(C, file = paste0(save_path, group, "_SE_pp_", se_pp, "_nroad_transects_", nroad_transects, 
                           "_SD_N_", sd_random_values, "_SD_ppd_", sd_ppd, 
...
for (census_data_index in seq_along(census_data)) {
  ...
  sink(paste("modelpppo(m)_census_data", census_data_index, ".txt", sep = ""))
  cat("
    model {
      # Priors
      ...
      for (i in 1:nroad_transects) {
        for (t in 1:nmonth) {
          ...
          N3[i, t] ~ dbin(pp, N[i, t])T(0, 100000)       # Roadkills preserved until road survey. We use truncation to avoid negative values.
  ...
  output <- jags(bdata, inits, params, paste("modelpppo(m)_census_data", census_data_index, ".txt", sep = ""),
...
for (sim in 1:nsimulations) {
  ...
  # Generate N3_itd matrix
  for (j in 1:D) {
    ppd_t <- if (j == 1) prod(ppd) else prod(ppd[j:length(ppd)])  # Adjust ppd according to to the age in days of the carcasses
...
for (sim in 1:nsimulations) {
  ...
  # Ensure the product of all ppd values is greater than 0.05. Since in this scenario there is no variability in ppd, we block this part of the code.
  #while (prod(ppd) <= 0.05) {
  #  ppd <- rnorm(D, mean = selected_group$ppd_group, sd = sd_ppd)
  #}
